...
export default function RentalDetailPage({
  ...
  useEffect(() => {
    ...
    rentalApi
      .get(id)
      .then((r) => {
        if (mounted) setRental(r);
      })
      .catch(() => {
        if (mounted) router.push("/rentals");
      })
      .finally(() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1">
              {rental.asset?.imageUrl ? (
                ...
                <div className="h-64 w-full rounded-lg bg-slate-100 flex items-center justify-center">
            ...
            <div className="md:col-span-2">
              ...
              <p className="mt-2 text-sm text-slate-600">
                {(rental as any).asset?.description}
              </p>
              ...
              {rental.notes && (
                <div className="mt-4 text-sm text-slate-500">
                  <strong>Notes:</strong>{" "}
                  <div className="italic">{rental.notes}</div>
                </div>
              )}
            </div>
          </div>
